...
const { checkPolicySafety } = require('./policySafety');
const { validateToolCall } = require('./toolGateway');
const { executeMongoQuery } = require('./mongoQueryExecutor');
 ...
 * @returns {Promise<{response: Object, selectedAgents: Array, confidence: number, requiresConfirmation: boolean}>}
 */
async function orchestrate(message, context) {
 ...
 * @param {string} message
 * @param {Object} context
 * @returns {Promise<{primaryIntent: string, confidence: number}>}
 */
 ...
 * @returns {string}
 */
function formatQueryResults(result, purpose) {
