...
async function resetAndCreateOrders() {
  try {
    ...
    // 기존 주문 삭제
    console.log('\n기존 주문 삭제 중...');
    const adminDeleted = await Order.deleteMany({ user: adminUser._id });
    const customerDeleted = await Order.deleteMany({ user: customerUser._id });
    console.log(`관리자 계정 주문 ${adminDeleted.deletedCount}건 삭제 완료`);
    console.log(`고객 계정 주문 ${customerDeleted.deletedCount}건 삭제 완료`);
