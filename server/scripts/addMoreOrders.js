    ...
    for (const { user, email } of users) {
      ...
      for (let i = 0; i < 35; i++) {
        ...
        const payment = {
          ...
          amount: grandTotal,
          currency: 'KRW',
          transactionId: `TXN${Date.now()}${i}${Math.random().toString(36).substring(7)}`,
          receiptUrl: '',
      ...
      console.log(`${email} 계정에 ${createdOrders.length}개의 주문이 추가되었습니다.`);
      ...
      console.log(`  상태별 주문: pending=${statusCounts.pending}, paid=${statusCounts.paid}, fulfilled=${statusCounts.fulfilled}, cancelled=${statusCounts.cancelled}`);
      console.log(`  총 주문 금액: ${totalAmount.toLocaleString()}원`);
    }

    // 전체 주문 현황 출력
    const totalAdminOrders = await Order.countDocuments({ user: adminUser._id });
    const totalCustomerOrders = await Order.countDocuments({ user: customerUser._id });
    const totalOrders = await Order.countDocuments();

    console.log('\n=== 전체 주문 현황 ===');
    console.log(`admin@admin.com: ${totalAdminOrders}건`);
    console.log(`ljh951006@naver.com: ${totalCustomerOrders}건`);
